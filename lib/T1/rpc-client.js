...
var responseTimeout = config.RESPONSE_TIMEOUT; //time to wait for response in ms
...
var RabbitMqClient = function(){
  ...
  return {
    ...
    makeRequest: function(routing_key, content, callback, timeout){
     // console.log('rpc_client, timeout - ' + timeout);
      var self = this;
      ...
      var tId = setTimeout(function(corr_id){
        ...
        //timely fashion
        callback(null, new Error("timeout expired, timeout - " + timeout));
        //delete the entry from hash
        delete self.requests[corr_id];
      }, timeout ? timeout : responseTimeout, correlationId);
